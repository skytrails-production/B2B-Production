...
// }
    ...
    const airlineCodeReturn = fareQuoteDataReturn?.Segments?.[0]?.[0]?.Airline?.AirlineCode;
    const flightNumberReturn = fareQuoteDataReturn?.Segments?.[0]?.[0]?.Airline?.FlightNumber;
    const originCityReturn = fareQuoteDataReturn?.Segments?.[0]?.[0]?.Origin?.Airport?.CityName;
    const DestinationCityReturn = fareQuoteDataReturn?.Segments?.[0]?.[0]?.Destination?.Airport?.CityName;
    const flightFareReturn = fareQuoteDataReturn?.Fare?.PublishedFare;
    const originTerminalReturn = fareQuoteDataReturn?.Segments?.[0]?.[0]?.Origin?.Airport?.Terminal;
    const destinationTerminalReturn = fareQuoteDataReturn?.Segments?.[0]?.[0]?.Destination?.Airport?.Terminal;


    const dateString3 = fareQuoteDataReturn?.Segments?.[0]?.[0]?.Origin?.DepTime;
    ...
    const dateString4 = fareQuoteDataReturn?.Segments?.[0]?.[0]?.Destination?.ArrTime;
    ...
    const Duration3 = `${Math.floor(fareQuoteDataReturn?.Segments?.[0]?.[0]?.Duration / 60)}hr ${fareQuoteDataReturn?.Segments?.[0]?.[0]?.Duration % 60
        }min`;
        ...
        <div className="col-lg-12">
            ...
            <div className="col-lg-12">
                <div className="confirmRect">
                    <div>
                        <p>THE SKYTRAILS PRIVATE LIMITED 1</p>
                        <span >PNR :{" "} {flightBookDataReturn?.PNR}</span>
                    </div>
                    <div>
                        <p>{flightBookDataReturn?.FlightItinerary?.Origin} -{" "}
                            {flightBookDataReturn?.FlightItinerary?.Destination}</p>
                        <span style={{ color: "#0048FF", fontWeight: "600" }}>Confirmed</span>
                    </div>
                    <div>
                        <p>Contact : {' '}{flightBookDataReturn?.FlightItinerary?.Passenger[0]?.ContactNo}</p>
                        <span>Book on: {dateFormat(ticket?.FlightItinerary?.LastTicketDate).date}</span>
                    </div>
                    <div>
                        <p>Travel Date:{" "}
                            {
                                dateFormat(flightBookDataReturn?.FlightItinerary?.Segments[0]?.Origin?.DepTime)
                                    ?.date
                            }</p>
                        <span> Travel Time:{" "}
                            {
            ...
            <div className="col-lg-12">
                <div className="singleFlightBox justify-content-evenly">
                    ...
                    <div className="singleFlightBoxTwo">
                        ...
                        <p style={{ fontSize: "14px" }}>{desiredFormat3.slice(13)}</p>
                        <p>Terminal{' '}{originTerminalReturn}</p>
                    </div>
                    <div className="singleFlightBoxThree">
                        <h4>{Duration3}</h4>
                        <div><img src={flightdir} /></div>
                    ...
                    <div className="singleFlightBoxFour">
                        <span>{DestinationCityReturn}</span>
                        {/* <p>{time2.substr(0, 5)}</p> */}
                        <p>{desiredFormat4.slice(0, 12)}</p>
                        <p style={{ fontSize: "14px" }}>{desiredFormat4.slice(13)}</p>
                        <p>Terminal{' '}{destinationTerminalReturn}</p>
                    </div>
                    <div className="singleFlightBoxFive">
                        <span>₹{flightFareReturn}</span>
                        <p>Publish</p>
                    </div>
                </div>
            </div>
            ...
            <div className="col-lg-12">
                {flightBookDataReturn?.FlightItinerary?.Segments?.map((flight, key) => (
                    <div className="fligtConfirmDetails">
                        <div>
                            <span>Flight No.</span>
                            <p>{flight?.Airline?.AirlineCode}-{flight?.Airline?.FlightNumber}</p>
...
export default FlightReturnConfirmationDetails;
