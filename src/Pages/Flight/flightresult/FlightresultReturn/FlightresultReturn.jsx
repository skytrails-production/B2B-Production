...
const FlightresultReturn = () => {
  ...
  if (
    ...
    destination = result[0][0]?.Segments[0][0]?.Destination?.Airport?.CityName;
  ...
  const minPrice1 = result ? result[1][0]?.Fare?.PublishedFare : 0;
  // console.log("minprice", minPrice);
  // console.log("minprice11111111111111111111", minPrice1);
  const maxPrice = result
    ? result[0][result[0]?.length - 1].Fare.PublishedFare
    : 100000;
  const maxPrice1 = result
    ? result[1][result[1]?.length - 1].Fare.PublishedFare
    : 100000;
  ...
  const Published_Fare = result
    ? result[0][result[0].length - 1].Fare.PublishedFare
    : null;
  const Published_Fare1 = result
    ? result[1][result[1].length - 1].Fare.PublishedFare
    : null;
  // console.log("Published_Fare", Published_Fare);
  const [priceRange, setPriceRange] = useState(Published_Fare);
  const [priceRange1, setPriceRange1] = useState(Published_Fare1);
  ...
  // const [selectedNumber, setSelectedNumber] = useState(maxPrice);
  const [selectedCategory, setSelectedCategory] = useState(["Price"]);
  const [selectedCategory1, setSelectedCategory1] = useState(["Price"]);
  const Airliness = [];
  const Airlines = [];

  // console.log("priceRange22222222222222222222222222222222222222222222222222222222",priceRange2)
  ...
  const clearFilter = () => {
    ...
    setSelectedCategory1(["Price"]);
    setTakeOffIndex(-1);
    setLandIndex(-1);
    setPriceRange(maxPrice);
    setPriceRange1(maxPrice1);
    setAirlinesIndex(-1);
  };
  ...
  useEffect(() => {
    const uniqueData1 = result
      ? result[1].filter((item, index, array) => {
          const isUnique1 = !array
            .slice(0, index)
            .some(
              (prevItem) =>
                prevItem.AirlineCode === item.AirlineCode &&
  ...
  // ////////////////////////////departure//////////////////////////
  const filteredDeparture = result
    ? FlightList1?.filter((item) => {
        const segmentdeparture = item?.Segments[0].length;
        const airlineName = item?.Segments[0][0]?.Airline?.AirlineName;

        // const airlineName1 = item?.Segments[0][0]?.Airline?.AirlineName;
        ...
        const categoryFilters = selectedCategory1.map((category) => {
          switch (category) {
            case "Price":
              return (
                minPrice1 <= item?.Fare?.PublishedFare &&
                item?.Fare?.PublishedFare <= priceRange1
              );
            case "startLandNon":
              return (
                segmentdeparture === 1 &&
            ...
            case "Airlines":
              return airlineName === Airliness[airlinesIndex]?.AirlineName;
            default:
              return 1 === 1;
          }
        });
  ...
  const filteredDeparturetakeof = result
        ...
        const categoryFilters = selectedCategory.map((category) => {
          switch (category) {
            ...
            case "startTakeoffNon":
              return segmentdeparture === 1;
            case "startTakeoffStop":
              return 2 <= segmentdeparture;
  ...
  const handleTakeOffChange = (index) => {
    if (takeOffIndex === -1 && takeOff) {
      ...
      setTakeOffIndex(-1);
    } else if (takeOffIndex !== index && takeOffIndex !== -1 && takeOff) {
      setSelectedCategory((pre) =>
        pre.filter(
  ...
  const handleLandChange = (index) => {
    if (landIndex === -1 && takeOff) {
      ...
      setLandIndex(-1);
    } else if (landIndex === -1 && !takeOff && landIndex !== index) {
      setSelectedCategory1((pre) =>
        pre.filter(
  ...
  const handleEndTakeOffChange = (index) => {
    if (endTakeOffIndex == -1) {
      setSelectedCategory1((pre) => [...pre, startTakeOffChangeList[1][index]]);
  ...
  // useEffect(() => {
  //   console.log(selectedCategory, "selectedCategory112121212121212121");
  // }, [selectedCategory]);
                    ...
                    <div className="innerFilter">
                        ...
                        <div class="wrapperRange">
                          ...
                          <div class="Inputrange">
                            <input
                              ...
                              max={maxPrice1}
                              value={priceRange1}
                              onChange={(e) => setPriceRange1(e.target.value)}
                              id="rangeInputPrice1"
                            />
                          </div>
                          <div className="price_range">
                      ...
                      <div>
                        <h2 className="sidebar-title">
                          Landing at{" "}
                          {takeOff
                            ? result[0][0]?.Segments[0][
                                result[0][0]?.Segments[0].length - 1
                              ]?.Destination?.Airport?.CityName
                            : result[1][0]?.Segments[0][
                                result[1][0]?.Segments[0].length - 1
                              ]?.Destination?.Airport?.CityName}
                        </h2>
            ...
            >
              <div className="col-lg-6">
                <FlightresultOne
                  ...
                  filteredDeparture={filteredDeparturetakeof}
                />
              </div>
              ...
              <div className="col-lg-6">
                ...
                <FlightReturn
                  filteredDeparture={filteredDeparture}
                  sendDataToParent={receiveChildData}
                />
                {/* </div> */}
                {/* </div> */}
