...
const ResultOnewayMain = () => {
  ...
  //   );
console.log(reducerState,standardizedFlights1,reducerState?.return?.returnData?.[0]?.journeyFlight ,"-------------------")
  ...
  useEffect(() => {
    ...
    if (!reducerState?.return?.isLoading) {
      ...
      //   );
      if (reducerState?.return?.isLoadingFilter) {
        setJornyFlights(reducerState?.return?.returnData?.[0]?.journeyFlight || []);
