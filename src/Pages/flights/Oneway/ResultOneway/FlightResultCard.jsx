  ...
  return (
    <div class="w-full relative transition-all ease-in-out hover:scale-[1.01] bg-white shadow-sm rounded-lg  p-3 px-3 hover:shadow-sm cursor-pointer border">
      ...
      >
        ...
        {/* <!-- Duration and Stops --> */}
        <div className="relative flex flex-col items-center justify-center flex-1 gap-2 group">
          {/* <p class="text-sm text-gray-500">{item?.destination}</p> */}
          <p class="font-semibold text-gray-700">{item?.layover}</p>
          <div className=" w-2/3 h-[1.5px]  mt-2" style={{backgroundColor:"#6366F1"}}>
            <div className="">
              {item.stopes !== 0 && (
                <div className="absolute w-2 h-2 transform -translate-x-1/2 -translate-y-1/2 bg-red-400 rounded-full left-1/2"></div>
              )}
              {/* Tooltip */}
              {item.stopes !== 0 && (
        ...
        <div className="flex flex-col items-end justify-center flex-1 gap-2">
          ...
          <button
            ...
            className="px-3 py-1 text-sm font-medium text-white rounded-full hover:bg-primary-6000"
