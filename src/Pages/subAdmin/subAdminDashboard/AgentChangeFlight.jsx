...
const AgentChangeFlight = () => {
  ...
  const handleShowAlert = (booking) => {
    ...
    const passengerDetailsHtml = rowDetails.flightDetails.passengerDetails
      .map(
        (passenger) => `
  ...
  const columns = [
...

    {
      ...
      width: 160,
      valueGetter: (params) =>
        params?.row?.flightDetails?.passengerDetails[0]?.ContactNo,
      // {
      //   const passenger1 = params.row.flightDetails.passengerDetails[0];
      //   const passenger2 = params.row.flightDetails.passengerDetails[1];
...
export default AgentChangeFlight;
