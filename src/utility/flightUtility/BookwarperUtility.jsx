...
export const saveDB = (type) => {
  ...
  const handleBookingDetails = (flight, type) => {
    ...
    if (flight.type === "TBO") {
      ...
      userApi.flightBookingDataSaveKafila(payload);
      // return ticket;
    } else if (flight.type === "AMD") {
      const flightDetails = flight?.flight;
      seat =
      ...
      //   nonStop
      // );
      userApi.flightbookingamedius(payload);
      // ticket.boardingTime = boardingTime || "2h before departure";
      // return ticket;
    }
...
export function findAirportByCode(code) {
  ...
  const data =airportList ? airportList?.find((airport) => airport?.AirportCode === code):{name:code};
